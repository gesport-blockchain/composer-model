...
var uri = "net.gesport"
var officeUri = "net.gesport.Office"
...
var partyController = {
    ...
    updateParty: async function(party) {
        ...
        if(!party.office) {
            ...
            if(offices.length==1) {
                ...
                party.officeId =  office.officeId
            ...
            let office = await officeRegistry.getId(party.officeId)
            party.organization = party.organization || office.organization
            party.organization.name = party.organization.name || office.organization.name
            party.address = party.address || office.address
            party.email = party.email || office.email
    ...
    getParticipantParty: async function(currentParticipant) {
        let officeRegistry = await getParticipantRegistry(officeUri)
        let office = await officeRegistry.get(currentParticipant.officeId)
        return office
    },
    ...
    checkOldParty: function(oldParty, newParty, oldParties)  {
        if(oldParty) {
            if(!newParty) {
                oldParties.push(oldParty.officeId)
    ...
    isPCS: function(office) {
        if(!office.types) return false
        return office.types.indexOf("PCS") > -1
    },

    isAgentOf: async function(agent, company) {
        if(!agent.officeId) return false
        if(!company.organization.code) return false
        let officeRegistry = await getParticipantRegistry(officeUri)
        let agentOffice = await officeRegistry.get(agent.officeId)
        return agentOffice.agentOf.indexOf(company.organization.code) > -1
    },
