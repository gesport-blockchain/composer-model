...
var containerController = {
     ...
     * @param {BLDeclaration} blAsset The bill of lading declaration
     * @param {BillOfLadingAsset} blAsset The bill of lading asset
     * @returns {Container} The container asset 
     */
    register: async function(cn, bl, blAssset) {
        ...
        if(exist) {
            ...
            switch(cnAsset.status) {
                case "REGISTERED":
                    ...
                    Object.assign(cnAsset, {
                        ...
                        containerType: cn.containerType,
                        fullIndicator: cn.fullIndicator,
                    ...
                    let event = factory.newEvent(uri,"BusinessEvent")
                    event.blAsset = blAsset
                    event.cnAsset = cnAsset
                    event.notifiedOffices = [cnAsset.dischargeShippingAgent.office, blAsset.carrier.office, cnAsset.dischargeTerminalOperator.office]
                    event.transactionName = "UpdateContainer"
                    event.assetId = cnAsset.cnId
                    emit(event)
                    return cnAsset
                default:
            ...
            containerRegistry.add(cnAsset)
            var event = eventController.newEvent("AddContainer", 
                            [blAssset.dischargeShippingAgent.office, blAsset.carrier.office, cnAsset.dischargeTerminalOperator.office],
                            blAsset.blId, cnAsset, blAsset, null)
            emit(event)
            return cnAsset
        }
    },
    ...
    remove: async function(cnNumber, blAsset) {
        ...
        if(exist) {
            let cnAsset = await containerRegistry.get(cnId)
            if(cnAsset.movements.length>0 || blAsset.deliveryOrder) {

            }
            switch(cnAsset.status) {
                case "REGISTERED":
                    containerRegistry.remove(cnAsset)
                    var event = eventController.newEvent("RemoveContainer", 
                                    [blAssset.dischargeShippingAgent.office, blAsset.carrier.office, cnAsset.dischargeTerminalOperator.office],
                                    blAsset.blId, cnAsset, blAsset, null)
                    emit(event)
                    return cnAsset
     ...
     * @param {Movement} movement The movement details
     */
    containerReturn: async function(cnId, movement, containerRegistry) {
        let blRegistry = await getAssetRegistry(blUri)
        let containerRegistry = await getAssetRegistry(cnUri)
        let blId = cnId.substr(cnId.indexOf("@")+1)
        var exist = await containerRegistry.exists(cnId)
